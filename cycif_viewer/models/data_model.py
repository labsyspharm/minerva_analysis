...
def load_ball_tree(datasource_name_name, reload=False):
    ...
    pickled_kd_tree_path = str(
        Path(
            os.path.join(os.getcwd())) / "cycif_viewer" / "static" / "data" / datasource_name_name / "ball_tree.pickle")
...
def get_color_scheme(datasource_name, refresh, label_field='phenotype'):
    color_scheme_path = str(
        Path(os.path.join(os.getcwd())) / "cycif_viewer" / "static" / "data" / datasource_name / str(
...
def get_similar_neighborhood_to_selection(datasource_name, selection, similarity):
    ...
    standard_neighborhoods = np.load(Path("cycif_viewer/static/data/Ton/complex_small.npy")).squeeze()
    ...
    if len(neighbor_ids) < 1000:
        ...
        neighborhood_array = np.load(Path("cycif_viewer/static/data/Ton/complex_medium.npy")).squeeze()
    else:
        neighborhood_array = np.load(Path("cycif_viewer/static/data/Ton/complex_large.npy")).squeeze()
