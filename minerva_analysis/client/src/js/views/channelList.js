...
class ChannelList {
    ...
    init(dd) {
        ...
        _.each(this.columns, (column) => {
            ...
            let svg = d3.select(svgCol).append("svg").attr("width", 15).attr("height", 15);
    ...
    addDownloadEvents() {
        ...
        channels_download_icon.addEventListener("click", () => {
            this.dataLayer.downloadChannelsCSV(imageChannelsIdx, this.currentChannels, this.colorConnector, this.rangeConnector, this.image_channels);
        });
        ...
        channels_download_icon_db.addEventListener("click", () => {
            this.dataLayer.saveChannelList(imageChannelsIdx, this.currentChannels, this.colorConnector, this.rangeConnector, this.image_channels);
    ...
    addSlider(name, swidth, activeRange) {
        ...
        let yScale = d3.scaleLinear().domain(yDomain).range([0, 23]);
        ...
        paths.attr("d", line).attr("class", "image_distribution_line").attr("transform", "translate(0,-31)").attr("fill", "none");

        gSimple.call(sliderSimple);

        //slider value to be displayed closer to the slider than default
        d3.selectAll(".parameter-value").select("text").attr("y", 10);
...
window.addEventListener("resize", function () {
    if (typeof channelList != "undefined" && channelList) {
        channelList.sliders.forEach((slider, name) => {
            d3.select("div#channel-slider_" + name)
                .select("svg")
                .remove();
            const channelListEl = document.getElementById("channel_list");
            const swidth = channelListEl.getBoundingClientRect().width;
