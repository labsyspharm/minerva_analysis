...
class CSVGatingList {
    ...
    init(dd) {
        ...
        _.each(this.columns, (column) => {
            ...
            let svg = d3.select(svgCol).append("svg").attr("width", 30).attr("height", 15);
        ...
        parent.addEventListener("dragleave", (ev) => {
            if (ev.x > rect.left + rect.width || ev.x < rect.left || ev.y > rect.top + rect.height || ev.y < rect.top) {
            ...
            targets.forEach((cLC) => {
                cLC.addEventListener("click", (e) => {
                    ...
                    if (!svgEls.includes(e.currentTarget.tagName)) {
                        ...
                        if (name) {
                            const match = Array.from(matches).find((gLC) => gLC.querySelector(`.${match_class}`).innerText === name);
    ...
    addSlider(name, swidth, data, activeRange) {
        ...
        let yScale = d3.scaleLinear().domain(yDomain).range([0, 25]);
        ...
        paths.append("path").attr("d", line).attr("class", "distribution_line").attr("transform", "translate(0,-31)").attr("fill", "none");
...
window.addEventListener("resize", () => {
    if (typeof csv_gatingList != "undefined" && csv_gatingList) {
        csv_gatingList.sliders.forEach((slider, name) => {
            d3.select("div#csv_gating-slider_" + name)
                .select("svg")
                .remove();
