...
raw_data = pd.read_csv("../static/data/tonsil_with_phenotype.csv")
...
temp = []
neighborhood_matrix = np.zeros((data.shape[0], len(phenotypes)))
A = data.to_numpy()
n = []
d = []
for i in tqdm(range(A.shape[0])):
    ...
    for k in range(len(neighbors)):
        ...
        neighbor_phenotype_index = phenotypes[neighbor_phenotype]
        weight = 1
        if distance > 25:
            weight = 1 - (distance - 25) / 25
        neighborhood_matrix[i, neighbor_phenotype_index] += weight

np.save('../static/data/neighborhood_array.npy', neighborhood_matrix)
